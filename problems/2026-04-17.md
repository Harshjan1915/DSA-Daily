...
diff
 of length 
n - 1
. Your task is to construct a sequence of length 
n
, denoted by 
a[0], a[1], ..., a[n - 1]
...
All elements in the sequence are 
non-negative
.
...
idx
 in the sequence must not exceed 
maxVal
 (i.e., 
a[idx] <= maxVal
).
...
Your goal is to construct a valid sequence that 
maximizes
 the 
largest
...
largest
 value present in such an optimal sequence.
...
n = 10, restrictions = [[3,1],[8,1]], diff = [2,2,3,1,4,5,1,1,2]
...
n = 8, restrictions = [[3,2]], diff = [3,5,2,4,2,3,1]
